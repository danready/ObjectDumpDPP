...
static int
V1742UnpackEventGroup (uint32_t group, uint32_t * datain,
...
dataout->DataChannel[2][wpnt] +=
	    (float) ((datain[rpnt + 1] & 0x0000000F) << 8);
...
dataout->DataChannel[5][wpnt] +=
	    (float) ((datain[rpnt + 1] & 0x000000FF) << 4);
...
dataout->DataChannel[8][wpnt] +=
	    (float) ((datain[rpnt + i] & 0x0000000F) << 8);
...
int32_t GetNumEvents (char *buffer, uint32_t buffsize,
			 uint32_t * numEvents)
{
...
int32_t GetEventPtr (char *buffer, uint32_t buffsize, int32_t numEvent,
			char **EventPtr)
{
...
if ((buffer == NULL) || (buffsize < EVENT_HEADER_SIZE))
    return -1;
...
Event =
    (CAEN_DGTZ_X742_EVENT_t *) malloc (sizeof (CAEN_DGTZ_X742_EVENT_t));
...
for (h = j - 1; h > -1; h++)
		    free (Event->DataGroup[g].DataChannel[h]);
